...
- Not run locally, but passing in CI: no Android, Apple or Flutter SDK is installed in the authoring environment, so the native sources were written and reviewed by inspection there. `.github/workflows/native.yml` then built all three adapters on hosted machines and they passed for the commit carrying the native source changes: the Android library build, the Apple package and iOS simulator builds, and Flutter `analyze` with the widget tests, which gained assertions without losing any. That is a compiling, test-passing build, which is a weaker claim than a tested device experience.
